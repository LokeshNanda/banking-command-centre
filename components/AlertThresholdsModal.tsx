...
export function AlertThresholdsModal({ isOpen, onClose, onSave }: AlertThresholdsModalProps) {
  const [thresholds, setThresholdsState] = useState<KpiThresholds>({});
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md glass-panel p-6 z-50"
          >
            <h3 className="text-lg font-bold text-white mb-4">Configurable Alert Thresholds</h3>
            <p className="text-sm text-slate-400 mb-6">
              Set warning and critical thresholds per KPI. Stored in browser.
            </p>
            <div className="space-y-4 max-h-[60vh] overflow-y-auto">
              {THRESHOLD_FIELDS.map(({ key, label, warningLabel, criticalLabel }) => {
                ...
                if (!t) return null;
                return (
                  <div key={key} className="space-y-2 p-3 rounded-lg bg-slate-800/30">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full mt-1 px-2 py-1.5 bg-slate-900 border border-slate-600 rounded text-sm text-white"
                        />
                      </div>
                      <div>
